...
#include "SickLMS5xxUtility.hh" // for byte-order conversions where necessary
...
namespace SickToolbox {
  ...
  void SickLMS5xxMessage::BuildMessage( const uint8_t * const payload_buffer, const unsigned int payload_length ) {
    ...
    for (int i = 0; i < 3; i++) {
		command_type[i] = payload_buffer[i];
    }
    command_type[3] = '\0';
    ...
    char command[15] = {0};
    int i = 0;
	for (; (i < min(14, payload_length-4)) && (payload_buffer[4 + i] != 0x20); i++) {
		command[i] = payload_buffer[4 + i];
    }
    command[i] = '\0';
    _command = command;
   ...
   * \param *message_buffer A well-formed message to be parsed into the class' fields
   */
  void SickLMS5xxMessage::ParseMessage( const uint8_t * const message_buffer ) throw (...) {
