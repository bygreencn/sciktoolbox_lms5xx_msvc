...
namespace SickToolbox {
	 ...
	 */
	void SickLMS5xxBufferMonitor::GetNextMessageFromDataStream(SickLMS5xxMessage &sick_message) throw(...) {
	 ...
	 */
	void SickLMS5xxBufferMonitor::_flushTCPRecvBuffer() const throw (...) {
		...
		for (int i = 0; i < num_bytes_waiting; i++) {
			...
			if (recv(_sick_fd, &null_byte, 1,0) != 1) {
				throw SickIOException("SickLMS5xxBufferMonitor::_flushTCPRecvBuffer: ioctl() failed!");
#else
			/* Capture a single byte from the stream! */
