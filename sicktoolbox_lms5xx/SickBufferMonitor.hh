...
namespace SickToolbox {
	...
	public:
		...
		SickBufferMonitor(SICK_MONITOR_CLASS * const monitor_instance) throw(...);

		/** A method for setting the target data stream */
		void SetDataStream(const unsigned int sick_fd) throw(...);

		/** Start the buffer monitor for the device */
		void StartMonitor(const unsigned int sick_fd) throw(...);

		/** Acquire the most recent message buffered by the monitor */
		bool GetNextMessageFromMonitor(SICK_MSG_CLASS &sick_message) throw(...);

		/** Stop the buffer monitor for the device */
		void StopMonitor() throw(...);

		/** Locks access to the data stream */
		void AcquireDataStream() throw(...);
		...
		void ReleaseDataStream() throw(...);

		/** A standard destructor */
		~SickBufferMonitor() throw(...);
	...
	protected:
		...
		void _readBytes(uint8_t * const dest_buffer, const int num_bytes_to_read, const unsigned int timeout_value = 0) const throw(...);
	...
	private:
		...
		void _acquireMessageContainer() throw(...);

		/** Unlocks access to the message container */
		void _releaseMessageContainer() throw(...);
	 ...
	 */
	template < class SICK_MONITOR_CLASS, class SICK_MSG_CLASS >
	SickBufferMonitor< SICK_MONITOR_CLASS, SICK_MSG_CLASS >::SickBufferMonitor(SICK_MONITOR_CLASS * const monitor_instance) throw(...) :
		_sick_monitor_instance(monitor_instance), _continue_grabbing(true) {
	 ...
	 */
	template < class SICK_MONITOR_CLASS, class SICK_MSG_CLASS >
	void SickBufferMonitor< SICK_MONITOR_CLASS, SICK_MSG_CLASS >::SetDataStream(const unsigned int sick_fd)throw() {
	 ...
	 */
	template < class SICK_MONITOR_CLASS, class SICK_MSG_CLASS >
	void SickBufferMonitor< SICK_MONITOR_CLASS, SICK_MSG_CLASS >::StartMonitor(const unsigned int sick_fd)throw(...) {
	 ...
	 */
	template < class SICK_MONITOR_CLASS, class SICK_MSG_CLASS >
	bool SickBufferMonitor< SICK_MONITOR_CLASS, SICK_MSG_CLASS >::GetNextMessageFromMonitor(SICK_MSG_CLASS &sick_message) throw(...) {
	 ...
	 */
	template < class SICK_MONITOR_CLASS, class SICK_MSG_CLASS >
	void SickBufferMonitor< SICK_MONITOR_CLASS, SICK_MSG_CLASS >::StopMonitor()throw(...) {
	 ...
	 */
	template < class SICK_MONITOR_CLASS, class SICK_MSG_CLASS >
	void SickBufferMonitor< SICK_MONITOR_CLASS, SICK_MSG_CLASS >::AcquireDataStream() throw(...) {
	 ...
	 */
	template < class SICK_MONITOR_CLASS, class SICK_MSG_CLASS >
	void SickBufferMonitor< SICK_MONITOR_CLASS, SICK_MSG_CLASS >::ReleaseDataStream() throw(...) {
	 ...
	 */
	template < class SICK_MONITOR_CLASS, class SICK_MSG_CLASS >
	SickBufferMonitor< SICK_MONITOR_CLASS, SICK_MSG_CLASS >::~SickBufferMonitor() throw(...) {
	 ...
	 */
	template < class SICK_MONITOR_CLASS, class SICK_MSG_CLASS >
	void SickBufferMonitor< SICK_MONITOR_CLASS, SICK_MSG_CLASS >::_acquireMessageContainer() throw(...) {
	 ...
	 */
	template < class SICK_MONITOR_CLASS, class SICK_MSG_CLASS >
	void SickBufferMonitor< SICK_MONITOR_CLASS, SICK_MSG_CLASS >::_releaseMessageContainer() throw(...) {
	...
	void SickBufferMonitor< SICK_MONITOR_CLASS, SICK_MSG_CLASS >::_readBytes(uint8_t * const dest_buffer, const int num_bytes_to_read, const unsigned int timeout_value) const
		...
		while (total_num_bytes_read < num_bytes_to_read) {
			...
			num_active_files = select(0, &file_desc_set, 0, 0, (timeout_value > 0) ? &timeout_val : 0);
			...
			if (num_active_files > 0) {
				...
				if (FD_ISSET(_sick_fd, &file_desc_set)) {
					...
					num_bytes_read = recv(_sick_fd, (char *)(&dest_buffer[total_num_bytes_read]), 1, 0);
#else
					/* Read a single byte from the stream! */
					num_bytes_read = red(_sick_fd, &dest_buffer[total_num_bytes_read], 1);
#endif
