...
namespace SickToolbox {
	...
	protected:
		...
		void _startListening() throw(...);

		/** Stops the driver from listening */
		void _stopListening()  throw(...);
		...
		void _setBlockingIO() const throw(...);

		/** Make the associated file descriptor non blocking */
		void _setNonBlockingIO() const throw(...);
		...
		void _recvMessage(SICK_MSG_CLASS &sick_message, const unsigned int timeout_value) const throw(...);
		...
		void _recvMessage(SICK_MSG_CLASS &sick_message,
			...
			const unsigned int timeout_value) const throw(...);
	 ...
	 */
	template< class SICK_MONITOR_CLASS, class SICK_MSG_CLASS >
	void SickLIDAR< SICK_MONITOR_CLASS, SICK_MSG_CLASS >::_startListening() throw(...) {
	 ...
	 */
	template< class SICK_MONITOR_CLASS, class SICK_MSG_CLASS >
	void SickLIDAR< SICK_MONITOR_CLASS, SICK_MSG_CLASS >::_stopListening() throw(...) {
	 ...
	 */
	template< class SICK_MONITOR_CLASS, class SICK_MSG_CLASS >
	void SickLIDAR< SICK_MONITOR_CLASS, SICK_MSG_CLASS >::_setBlockingIO() const throw(...) {
#if defined(_MSC_VER)
		u_long flags = 0;
		if (NO_ERROR != ioctlsocket(_sick_fd, FIONBIO, &flags))
			throw SickIOException("SickLIDAR::_setNonBlocking: ioctlsocket failed!");
#else
		/* Read the flags */
		int fd_flags = 0;
	 ...
	 */
	template< class SICK_MONITOR_CLASS, class SICK_MSG_CLASS >
	void SickLIDAR< SICK_MONITOR_CLASS, SICK_MSG_CLASS >::_setNonBlockingIO() const throw (...) {
#if defined(_MSC_VER)
		u_long flags = 1;
		if (NO_ERROR != ioctlsocket(_sick_fd, FIONBIO, &flags))
			throw SickIOException("SickLIDAR::_setNonBlocking: ioctlsocket failed!");
	...
	void SickLIDAR< SICK_MONITOR_CLASS, SICK_MSG_CLASS >::_sendMessage(const SICK_MSG_CLASS &sick_message, const unsigned int byte_interval) const
		...
		if (byte_interval == 0) {
			...
			if (send(_sick_fd, (const char *)message_buffer, message_length, 0) == SOCKET_ERROR) {
				throw SickIOException("SickLIDAR::_sendMessage: send() failed!");
			}
#else
			...
			for (unsigned int i = 0; i < message_length; i++) {
				...
				if (send(_sick_fd, (const char *)(&message_buffer[i]), 1, 0) == SOCKET_ERROR) {
					throw SickIOException("SickLIDAR::_sendMessage: send() failed!");
				}
#else
				/* Write a single byte to the stream */
				if (send(_sick_fd, &message_buffer[i], 1) != 1) {
					throw SickIOException("SickLIDAR::_sendMessage: write() failed!");
				}
#endif
