...
namespace SickToolbox {
	...
	public:
		...
		void Initialize(const bool disp_banner = true) throw(...);
		...
		void SetSickEchoFilter(sick_lms_5xx_echo_filter_t echo_filter) throw(...);

		/** Get the Sick LMS 5xx scan frequency */
		sick_lms_5xx_scan_freq_t GetSickScanFreq() const throw(...);

		/** Get the Sick LMS 5xx scan resolution */
		sick_lms_5xx_scan_res_t GetSickScanRes() const throw(...);

		/** Get the Sick LMS 5xx scan start angle */
		double GetSickStartAngle() const throw(...);

		/** Get the Sick LMS 5xx scan stop angle */
		double GetSickStopAngle() const throw(...);

		/** Sets the sick scan data format */
		void SetSickScanDataFormat(const sick_lms_5xx_scan_format_t scan_format) throw(...);
		...
		void GetSickMeasurements(unsigned int * const range_1_vals,
			...
			unsigned int * const dev_status = NULL) throw(...);

		/** Uninitializes the Sick LD unit */
		void Uninitialize(const bool disp_banner = true)throw(...);
	...
	private:
		...
		void _setupConnection() throw(...);

		/** Re-initialized the device */
		void _reinitialize() throw(...);

		/** Teardown the connection to the Sick LD */
		void _teardownConnection() throw(...);

		/** Acquire the latest Sick LMS's status */
		void _updateSickStatus() throw(...);

		/** Acquire the Sick LMS's scan config */
		void _getSickScanConfig() throw(...);
		...
		void _setAuthorizedClientAccessMode() throw(...);

		/** Save configuration parameters to EEPROM */
		void _writeToEEPROM() throw(...);

		/** Send the message w/o waiting for a reply */
		void _sendMessage(const SickLMS5xxMessage &send_message) const throw(...);
		...
		void _sendMessageAndGetReply(const SickLMS5xxMessage &send_message,
			...
			const unsigned int num_tries = 1) throw(...);

		/** Receive a message */
		void _recvMessage(SickLMS5xxMessage &sick_message) const throw(...);

		/** Start device measuring */
		void _startMeasuring() throw(...);

		/** Stop device measuring */
		void _stopMeasuring() throw(...);

		/** Request a data data stream type */
		void _requestDataStream() throw(...);

		/** Start streaming measurements */
		void _startStopStreamingMeasurements(bool start) throw(...);

		/** Set device to measuring mode */
		void _checkForMeasuringStatus(unsigned int timeout_value = DEFAULT_SICK_LMS_5XX_STATUS_TIMEOUT) throw(...);

		/** Sets the sick scan data format */
		void _setSickScanDataFormat(const sick_lms_5xx_scan_format_t scan_format) throw(...);

		/** Sets the Sick LMS 5xx echo filter mode */
		void _setSickEchoFilter(sick_lms_5xx_echo_filter_t echo_filter) throw(...);

		/** Restore device to measuring mode, i.e., logout of configuration mode */
		void _restoreMeasuringMode() throw(...);
